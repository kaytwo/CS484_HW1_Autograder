...
try{
    createDatabase((err)=>{
        ...
        createServiceRequestTable((res)=>{
            if (res == true){
                console.log(`Created service_request Table inside ${dbName}.db`);
            }else{
                console.log(`Unable to create service_request Table inside ${dbName}.db`);
            }
        })
...
function createServiceRequestTable(callback){

    sqlQuery = "CREATE TABLE service_request ("
    +"    name         VARCHAR (200),"
